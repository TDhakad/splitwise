...
export default function FriendsView({ users, rawBalances, balances, currentUserId, onSettleUp, onSelectFriend, onRefresh }) {
  ...
  const handleSendRequest = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAddError(data.detail || 'Failed to send request');
      }
    } catch (e) {
  ...
  const handleRespondRequest = async (id, status) => {
    try {
      const res = await apiFetch(`/friends/request/${id}?status=${status}`, { method: 'PUT' });
      if (res.ok) {
        if (onRefresh) onRefresh(); // Trigger global refresh to get new users list
        fetchRequests();
      }
    } catch (e) {
      console.error(e);
  ...
  const friendsList = Object.entries(friendBalances).map(([id, net]) => {
            ...
            <div className="p-6">
              ...
              <form onSubmit={handleSendRequest}>
                ...
                <div className="flex gap-3 justify-end">
                  ...
                  <button type="submit" disabled={isAdding || !addEmail.trim()} className="px-5 py-2.5 font-bold text-sm text-white bg-[#007A64] hover:bg-[#00604f] rounded-xl transition-colors disabled:opacity-50">
