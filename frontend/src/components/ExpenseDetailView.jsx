...
export default function ExpenseDetailView({ expense, context, users, currentUserId, refreshTrigger, onBack, onEdit }) {
   ...
   useEffect(() => {
      if (expense?.id) {
         apiFetch(`/expenses/${expense.id}/audit`)
            .then(r => r.json())
            .then(setAuditLogs)
            .catch(console.error);
      }
   }, [expense.id, refreshTrigger]);
   ...
   const formatMonth = (d) => new Date(d).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });

   // Generate breadcrumbs based on context
   let breadcrumbs = [];
   let backText = "Back";
   if (context?.from === 'group') {
      breadcrumbs = ['Groups', context.groupName, expense.description];
      backText = `Back to ${context.groupName}`;
   } else {
