...
import type { ActivityItem, ExpenseActivity, GroupInviteActivity, SettlementActivity } from '../types/ui';

interface ExpenseActivityRowProps {
      ...
      <div onClick={onClick} className="p-5 flex items-start gap-4 hover:bg-gray-50 transition-colors cursor-pointer group">
         ...
         <div className="text-right shrink-0 pt-1 ml-4">
            ...
            {Math.abs(activity.net) <= 0.005 && (
               <>
                  {activity.expenseObj?.participants?.length === 1 && activity.expenseObj.participants[0].user_id === currentUserId ? (
                     <>
                        <p className="text-[11px] font-bold tracking-widest uppercase text-gray-500 mb-1">Personal</p>
                        <p className="text-xl font-bold text-gray-700">${activity.expenseObj.total_amount.toFixed(2)}</p>
                     </>
                  ) : (
