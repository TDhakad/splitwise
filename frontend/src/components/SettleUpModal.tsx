...
import { useCreateSettlement } from '../features/settlements/api';
...
export default function SettleUpModal({ onClose, onSave, users, currentUserId, defaultPayerId, defaultPayeeId, defaultAmount, defaultMaxAmount, defaultGroupId }: SettleUpModalProps) {
   const [payerId] = useState(defaultPayerId || currentUserId);
   const [payeeId, setPayeeId] = useState<number | ''>(defaultPayeeId || '');
   ...
   const payeeUser = typeof payeeId === 'number' ? users.find(u => u.id === payeeId) : null;
   const isDirectionLocked = Boolean(defaultPayerId && defaultPayeeId);

   const availablePayees = users.filter(u => u.id !== payerId);
   const parsedAmount = Number(amount);
   const exceedsMax = maxAmount > 0 && Math.round(parsedAmount * 100) > Math.round(maxAmount * 100);
   const canSubmit = Boolean(payerId && payeeId && payerId !== payeeId && amount && !Number.isNaN(parsedAmount) && parsedAmount > 0 && !exceedsMax);

   const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      if (!canSubmit || typeof payeeId !== 'number') return;
      ...
      try {
         ...
         console.error(err);
         alert(getErrorMessage(err, 'Error recording settlement.'));
      }
   };
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
               ...
               <div className="flex items-center justify-between bg-[#F8F9FA] p-4 rounded-xl border border-gray-100">
                  <div className="flex flex-col items-center gap-2">
                     <div className={clsx("w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-sm border-2 border-white", avatarColor(payerId))}>
                        {initials(payerUser?.name || 'User')}
                     </div>
                     <span className="text-xs font-bold text-gray-600">{payerId === currentUserId ? 'You' : payerUser?.name || 'Payer'}</span>
                  </div>
                  ...
                  <div className="flex flex-col items-center gap-2">
                     {payeeId ? (
                        <div className={clsx("w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold shadow-sm border-2 border-white", avatarColor(payeeId))}>
                  ...
                  <div>
                     ...
                     <div className="relative">
                        <CustomDropdown
                           ...
                           renderSelected={(opt) => {
                              const u = users.find(usr => usr.id === Number(opt.value));
                              return (
                                 <>
                                    <div className={clsx("w-8 h-8 rounded-full flex items-center justify-center font-bold text-white text-xs", avatarColor(Number(opt.value)))}>
                                       {initials(u?.name || 'U')}
                                    </div>
               ...
               <div className="pt-4 flex gap-3">
                  ...
                  <button type="submit" disabled={createSettlement.isPending || !canSubmit} className="flex-1 px-4 py-3 rounded-xl font-bold text-white bg-[#007A64] hover:bg-[#00604f] transition-colors disabled:opacity-50 flex justify-center items-center gap-2">
                     {createSettlement.isPending ? <MSIcon name="refresh" className="animate-spin" /> : <MSIcon name="check" />}
                     Record Payment
                  </button>
               </div>
